...
class Auth extends CI_Controller
{
    ...
    public function registration()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            congralation! your account has been created
        ...
        private function _sendEmail($token, $type)
        {
            ...
            $config['smtp_user'] = 'naylanurafida27@gmail.com';// isi dng email
            $config['smtp_pass'] = 'wjseggxduwjpvwix';
            $config['smtp_port'] = 465;
            $config['mailtype'] = 'html';
            $config['charset'] = 'utf-8';
            $this->email->initialize($config);
            $this->email->set_newline("\r\n");
            $this->email->from('naylanurafida27@gmail.com','Web programing SMKN8Jember');
            $this->email->to($this->input->post('email'));

            if ($type == 'verify') {
                $this->email->subject('Account Verivication');
                $this->email->message('Clik this link to verify yoou account :
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                    ...
                    $email . ' has been Activated!Pleae login</div>');
                    redirect('auth');
                } else {
                    ...
                    $this->session->set_flashdata(
                        ...
                        Account activation failed!Wrong taken!</div>');
                        redirect('auth');
                    }
                } else {
                    $this->session->set_flashdata(
